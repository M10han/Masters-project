...
class RBM(object):
    ...
    def transform(self,v, sess=None):
        if sess is None:
            ...
            z = np.add(np.matmul(v, sess.run(self.W)), sess.run(self.bh))
            prob_h=1/(1+np.exp(-z)) # compute
            state_h= (np.random.uniform(size=(v.shape[0],self.n_h))<prob_h).astype(np.float32) # sample
#        prob_h=z
